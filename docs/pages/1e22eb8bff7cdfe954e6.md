...
Source: https://core.telegram.org/api/action-bar
...
What follows is a list of the various (*mutually exclusive*) chat bar types, along with the **condition** used to identify each type, by checking the appropriate flags of [peerSettings](https://core.telegram.org/constructor/peerSettings).
...
The above information can come in handy for users to detect and [block](https://core.telegram.org/api/action-bar#report-spam-block-or-add-contact) spam or recently hijacked accounts.
...
This bar indicates that the associated private (non-secret) chat *with another user* is currently being managed by a [business bot connected to our current account »](https://core.telegram.org/api/bots/connected-business-bots).
...
* `business_bot_id` - Contains the ID of the [business bot »](https://core.telegram.org/api/bots/connected-business-bots) managing this chat, used to display info about the bot in the action bar.
...
* `business_bot_can_reply` - Whether the business bot can reply to messages in this chat, as specified by the settings during [initial configuration](https://core.telegram.org/api/bots/connected-business-bots).
...
See [here »](https://core.telegram.org/api/bots/connected-business-bots) for more info on business bots.
