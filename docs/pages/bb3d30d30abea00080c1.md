...
Source: https://core.telegram.org/api/invoking
...
Clients should treat this as a `500` server error, and handle it by closing and reopening the TCP socket, re-initializing the session with [initConnection](https://core.telegram.org/method/initConnection) and invoking [getDifference](https://core.telegram.org/api/updates).

#### [List of Available Layers](https://core.telegram.org/api/layers)
...
1. msg_id=4; `messages.sendMessage message=b` (resending the old query with `msg_id=2`)
2. msg_id=5; `invokeAfterMsg msg_id=4 (messages.sendMessage message=c)` (resending the old query with `msg_id=3`)
...
1. msg_id=4; `messages.sendMessage message=c` (resending the old query with `msg_id=3`)
...
An even simpler option is to always follow [Scenario 1](https://core.telegram.org/api/invoking#scenario-1), never using `invokeAfterMsgs` and only using chained `invokeAfterMsg` calls, thus avoiding the use of this slightly more complicated recovery logic.
...
Before transmitting a query, the string containing the entire body of the serialized high-level query (starting with the method number) must be compressed using gzip. If the resulting string is smaller than the original, it makes sense to transmit the [gzip_packed](https://core.telegram.org/mtproto/service_messages#packed-object) constructor.
...
By default, the server can compress the response to any request as well as [updates](https://core.telegram.org/api/updates), in accordance with the rules stated above. If the [gzip_packed](https://core.telegram.org/mtproto/service_messages#packed-object) constructor is received as a response in rpc_result, then the string that follows must be extracted and uncompressed. Processing then continues on the resulting new string.