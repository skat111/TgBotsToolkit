...
Source: https://core.telegram.org/api/paid-media
...
Creators can then [withdraw Stars using the Gram cryptocurrency »](https://core.telegram.org/api/stars#withdrawing-revenue), or use them to [advertise their channel](https://core.telegram.org/api/stars#paying-for-ads) and get even more subscribers – all of this with next to **0 commission** from Telegram.
...
* In `stars_amount`, the amount of [Telegram Stars](https://core.telegram.org/api/stars) users must pay to obtain access to the media.  
  The maximum value that can be passed here is specified in the [stars_paid_post_amount_max client configuration value »](https://core.telegram.org/api/config#stars-paid-post-amount-max).
* In `extended_media`, the actual media files (currently only photos and videos are supported).
  To send albums, do **not** use [messages.sendMultiMedia](https://core.telegram.org/method/messages.sendMultiMedia), but rather pass all the medias in the `extended_media` array.
