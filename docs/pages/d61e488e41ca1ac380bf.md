...
Source: https://core.telegram.org/api/factcheck
...
Those methods can be used by independent fact-checkers, which will have the [appConfig.can_edit_factcheck](https://core.telegram.org/api/config#can-edit-factcheck) configuration flag set to `true`.

If the flag mentioned above is set to `true`, the maximum UTF-8 length for a fact check will be specified in the [appConfig.factcheck_length_limit](https://core.telegram.org/api/config#factcheck-length-limit) field.