...
Note that writing `E = E_1 E_2 ... E_n` in the expression for *expr* means applying the function *E\_1* to the argument *E\_2*, applying the result to *E\_3*, etc. Specifically, `E_1 E_2 E_3 = (E_1 E_2) E_3`. A solitary `#` is included in *type-ident*, because it is actually the identifier for a built-in type (`#` alias `nat`).
