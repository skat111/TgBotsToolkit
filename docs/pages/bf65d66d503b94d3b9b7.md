...
* The [peer ID »](https://core.telegram.org/api/peers#peer-id)
* The [access hash »](https://core.telegram.org/api/peers#access-hash)
* [Other info »](https://core.telegram.org/api/peers#other-info)

And it must be populated as follows:

* [By saving received user, chat, channel constructors »](https://core.telegram.org/api/peers#saving-constructors)
* [By handling certain updates »](https://core.telegram.org/api/peers#handling-updates)
* [By manual refreshes »](https://core.telegram.org/api/peers#manual-refreshes)
...
The following updates should trigger an update of a small subset of the info contained in the peer database (both non-full and [full variants](https://core.telegram.org/api/peers#full-info-database)).
...
All updates related to [basic groups](https://core.telegram.org/api/channel#basic-groups) (not supergroups) also have a `version` integer field, which should be used to apply the update, comparing it with the `version` field of the cached [chat](https://core.telegram.org/constructor/chat) (from now on, referred to as `cached_version`):

* If `version` is smaller than `cached_version+1`, skip the update
* If `version` is equal to `cached_version+1`, apply the update and update the `version` of the local [chat](https://core.telegram.org/constructor/chat)
* If `version` is bigger than `cached_version+1`, skip the update, and invalidate and refetch the local [chat](https://core.telegram.org/constructor/chat) and [chatFull](https://core.telegram.org/constructor/chatFull) using [messages.getFullChat](https://core.telegram.org/method/messages.getFullChat)
* If there is no locally cached [chat](https://core.telegram.org/constructor/chat) for this chat, skip the update and fetch full info using using [messages.getFullChat](https://core.telegram.org/method/messages.getFullChat)

Note that [chatParticipants](https://core.telegram.org/constructor/chatParticipants).`version` (contained in [chatFull](https://core.telegram.org/constructor/chatFull)) points to exactly the same sequence contained in [chat](https://core.telegram.org/constructor/chat).`version`.

Here's the full list of updates related to basic groups which have the `version` field, and which parts of the stored [chat](https://core.telegram.org/constructor/chat)/[chatFull](https://core.telegram.org/constructor/chatFull) should they update if applied:

* [updateChatParticipants](https://core.telegram.org/constructor/updateChatParticipants) - Update [chat](https://core.telegram.org/constructor/chat).`participants_count`, [chatFull](https://core.telegram.org/constructor/chatFull).`participants` (note that the `version` is contained in the [chatParticipants](https://core.telegram.org/constructor/chatParticipants) inside [updateChatParticipants](https://core.telegram.org/constructor/updateChatParticipants).`participants`, not in the top constructor, and is not present in the banned [chatParticipantsForbidden](https://core.telegram.org/constructor/chatParticipantsForbidden) variant).
...
* [updateChatParticipantAdd](https://core.telegram.org/constructor/updateChatParticipantAdd) - Update [chat](https://core.telegram.org/constructor/chat).`participants_count`, [chatFull](https://core.telegram.org/constructor/chatFull).`participants`.
* [updateChatParticipantDelete](https://core.telegram.org/constructor/updateChatParticipantDelete) - Update [chat](https://core.telegram.org/constructor/chat).`participants_count`, [chatFull](https://core.telegram.org/constructor/chatFull).`participants`.
* [updateChatParticipantAdmin](https://core.telegram.org/constructor/updateChatParticipantAdmin) - Update [chat](https://core.telegram.org/constructor/chat).`admin_rights` if `user_id` points to the current user and [chatFull](https://core.telegram.org/constructor/chatFull).`participants` unconditionally.
* [updateChatDefaultBannedRights](https://core.telegram.org/constructor/updateChatDefaultBannedRights) - Update [chat](https://core.telegram.org/constructor/chat).`default_banned_rights`
* [updateChatParticipantRank](https://core.telegram.org/constructor/updateChatParticipantRank) - Update the [ChatParticipant](https://core.telegram.org/type/ChatParticipant).`rank` (contained in [chatFull](https://core.telegram.org/constructor/chatFull).`participants`) of the specified user

An exception to the rule is [updateChatParticipant](https://core.telegram.org/constructor/updateChatParticipant): this update can only be used by bots, and does **not** have a `version` field; instead, deduplication relies only on the usual `qts` deduplication mechanism used for bot updates, see [here »](https://core.telegram.org/api/updates) for more info.

#### Manual refreshes

For performance reasons, the server will not always send updates containing updated information about all peers: for this reason, information about *already cached* peers may be refreshed manually in certain conditions using the bulked [users.getUsers](https://core.telegram.org/method/users.getUsers), [messages.getChats](https://core.telegram.org/method/messages.getChats), [channels.getChannels](https://core.telegram.org/method/channels.getChannels) methods, all requiring the previously cached `access_hash`.
...
* [Users »](https://core.telegram.org/api/bots/ids#user-ids)
* [Chats »](https://core.telegram.org/api/bots/ids#chat-ids)
* [Supergroups/channels »](https://core.telegram.org/api/bots/ids#supergroup-channel-ids)
* [Monoforums »](https://core.telegram.org/api/bots/ids#monoforum-ids)
...
The `access_hash` is the second most important field stored in the [peer database](https://core.telegram.org/api/peers#peer-info-database), used to generate [InputPeer](https://core.telegram.org/type/InputPeer), [inputUser](https://core.telegram.org/type/InputUser), [inputChannel](https://core.telegram.org/type/InputChannel) constructors used to interact with peers in the API.  
...
As specified in the constructor docs, some of the fields must not be overwritten if a [min constructor](https://core.telegram.org/api/min) is received, and a change in some other fields must trigger invalidation of the [full info database »](https://core.telegram.org/api/peers#full-info-database).
...
* Some event (specified [here »](https://core.telegram.org/api/peers#peer-info-database)) changes the value of a very specific subset of fields of an entry in the (non-full!) [peer info database »](https://core.telegram.org/api/peers#peer-info-database).  
...
* After invoking [messages.setChatWallPaper](https://core.telegram.org/method/messages.setChatWallPaper) to *remove* the wallpaper and receiving an error, for the peer whose wallpapers we tried to change, to fetch the correct and updated wallpaper settings.
...
* After invoking any of the following methods (successfully or not):
  ...
  + [messages.editChatCreator](https://core.telegram.org/method/messages.editChatCreator) - For the channel/supergroup/group
...
* If we banned a chat/supergroup participant specifying a [chatBannedRights](https://core.telegram.org/constructor/chatBannedRights).`until_date`, the full cache of the chat/supergroup should be refreshed for the admin at `until_date`.
