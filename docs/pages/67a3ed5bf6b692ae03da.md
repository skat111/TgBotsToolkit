...
| y_scaled | Indicates that each of the two (!) lines in a step graph must be visualized on its own scale, with two different tick axes on the left and right parts of the graph (boolean, see [various graphs](https://core.telegram.org/api/stats#graph-examples) for examples) |
