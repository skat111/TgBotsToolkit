...
Source: https://core.telegram.org/cdn
...
> **See also:** [More about CDNs and governments in the Advanced FAQ](https://core.telegram.org/techfaq#encrypted-cdns)
...
[![](/file/811140934/1815/aSQ-nSUHJzc.45662/40c2955965574cef80 "CDNs explained")](https://core.telegram.org/file/811140934/1815/aSQ-nSUHJzc.45662/40c2955965574cef80)
...
* Client developers can use [help.getCdnConfig](https://core.telegram.org/method/help.getCdnConfig) to obtain a list of public RSA keys for CDN DCs, which are different from public RSA keys of the master DCs.
* CDNs support only the following methods: [upload.getCdnFile](https://core.telegram.org/method/upload.getCdnFile), [initConnection](https://core.telegram.org/method/initConnection), [invokeWithLayer](https://core.telegram.org/method/invokeWithLayer).
...
* Clients must not send private user info that is passed in [initConnection](https://core.telegram.org/method/initConnection) to the CDNs.

### Getting files from a CDN

The API may return the [upload.fileCdnRedirect](https://core.telegram.org/constructor/upload.fileCdnRedirect) constructor after an [upload.getFile](https://core.telegram.org/method/upload.getFile) query. In this case, the client must request the required file from a CDN DC. The *dc\_id* in the response is the ID of the new CDN. The IP address for the connection will be available in [help.getConfig](https://core.telegram.org/method/help.getConfig), as with the master DCs. The corresponding [dcOption](https://core.telegram.org/constructor/dcOption) will have the flag *cdn:flags.3?true*.

Once a successful connection to the CDN DC identified by *dc\_id* is established, the client must generate an auth\_key (after confirming that the public RSA MTProto key of the CDN DC matches one from the list returned in [help.getCdnConfig](https://core.telegram.org/method/help.getCdnConfig)). Then the client must perform an [upload.getCdnFile](https://core.telegram.org/method/upload.getCdnFile) query for each *offset*. For files of unknown size, it is necessary to repeat the query until an empty reply is returned.

[upload.getCdnFile](https://core.telegram.org/method/upload.getCdnFile) may return the [upload.cdnFileReuploadNeeded](https://core.telegram.org/constructor/upload.cdnFileReuploadNeeded) constructor. In this case, the client needs to send an [upload.reuploadCdnFile](https://core.telegram.org/method/upload.reuploadCdnFile) request to the DC that got the original [upload.getFile](https://core.telegram.org/method/upload.getFile) request. Once [upload.reuploadCdnFile](https://core.telegram.org/method/upload.reuploadCdnFile) is successful, the app needs to request the file from the CDN DC again.

The main DC for a file is the DC where its main copy is stored (not to be confused with the main DC of the user) – either *userProfilePhoto.dc\_id*, *chatPhoto.dc\_id*, *photo.dc\_id*, or *document.dc\_id*.

### Decrypting files

In [upload.fileCdnRedirect](https://core.telegram.org/constructor/upload.fileCdnRedirect), the server sends a decryption key and IV for the file (the fields `encryption_key:bytes` and `encryption_iv:bytes` respectively).

Having received a portion of encrypted data from the CDN DC inside [upload.cdnFile](https://core.telegram.org/constructor/upload.cdnFile), the client must decrypt this data using `AES-256-CTR`. For IV, it should use the value of `encryption_iv`, modified in the following manner: for each `offset`, replace the last 4 bytes of the `encryption_iv` with `offset / 16` in big-endian. This allows clients to decrypt a file efficiently and use random access to the file's content (e.g., for streaming).

### Verifying files

In order to confirm that the CDN DC passed an untampered file, clients must verify hashes for each downloaded part. [upload.fileCdnRedirect](https://core.telegram.org/constructor/upload.fileCdnRedirect), [upload.reuploadCdnFile](https://core.telegram.org/method/upload.reuploadCdnFile) and [upload.getCdnFileHashes](https://core.telegram.org/method/upload.getCdnFileHashes) contain [FileHash](https://core.telegram.org/type/FileHash) constructors. Each of these constructors contains the SHA-256 hash of a part of the file that starts with `offset` and takes `limit` bytes.

Before saving each portion of the data received from the CDN DC into the file, the client must confirm that its hash matches the hash that was received from the master DC. If a hash is missing for any file part, client developers must use the [upload.getCdnFileHashes](https://core.telegram.org/method/upload.getCdnFileHashes) method to obtain the missing hash.
...
| --- | --- | --- |
| upload.getCdnFile | FILE\_TOKEN\_INVALID | The CDN DC did not accept the `file_token` (e.g., the token has expired). Continue downloading the file from the master DC using `upload.getFile`. |
| upload.reuploadCdnFile | FILE\_TOKEN\_INVALID | The master DC did not accept the `file_token` (e.g., the token has expired). Continue downloading the file from the master DC using `upload.getFile`. |
| upload.reuploadCdnFile | REQUEST\_TOKEN\_INVALID | The master DC did not accept the `request_token` from the CDN DC. Continue downloading the file from the master DC using `upload.getFile`. |

### Testing CDN redirects

You may test CDN file redirects by logging into the [test DCs](https://core.telegram.org/api/auth#test-accounts) with a DC 2 account (phone number `999662YYYY`, where YYYY are random numbers, login code `22222`), uploading and then redownloading the uploaded file.

Make sure the chosen test account doesn't have a Telegram Premium test subscription, as CDN file downloads are disabled on the test DCs if a Telegram Premium test subscription is enabled (you can check this by making sure the [user](https://core.telegram.org/constructor/user) returned by a [users.getUsers](https://core.telegram.org/method/users.getUsers) call with [inputUserSelf](https://core.telegram.org/constructor/inputUserSelf) does not have the `premium` flag set).