...
Source: https://core.telegram.org/api/stories
...
Note that in order to obtain permission to post stories as a channel/supergroup, it must be boosted, first, see [here »](https://core.telegram.org/api/boost) for more info.

[stories.canSendStory](https://core.telegram.org/method/stories.canSendStory) returns a [stories.canSendStoryCount](https://core.telegram.org/constructor/stories.canSendStoryCount) with the number of available active story slots (equal to the value of the [`story_expiring_limit_*` client configuration parameter](https://core.telegram.org/api/config#story-expiring-limit-default) minus the number of currently active stories) only if:
...
* `expire`: Period after which the story is moved to [archive](https://core.telegram.org/api/stories#pinned-or-archived-stories) (and [to the profile](https://core.telegram.org/api/stories#pinned-or-archived-stories) if `pinned` is set), in seconds; must be one of `6 * 3600`, `12 * 3600`, `86400`, or `2 * 86400` for Telegram Premium users, and `86400` otherwise.
* `pinned`: Whether to also [add the story to the profile](https://core.telegram.org/api/stories#pinned-or-archived-stories) automatically upon expiration. If not set, the story will only be added to the [archive](https://core.telegram.org/api/stories#pinned-or-archived-stories).
* `caption` and `entities`: The story caption, and related [styled text entities](https://core.telegram.org/api/entities); note that the story caption length is limited by the [`story_caption_length_limit_*` »](https://core.telegram.org/api/config#story-caption-length-limit-default) config keys, and story entities should only be sent and displayed according to the value of the [stories_entities »](https://core.telegram.org/api/config#stories-entities) config key.
...
Also, posting a story will emit an [updateStory](https://core.telegram.org/constructor/updateStory) both for us, and for our subscribers/contacts (even if they have [hidden](https://core.telegram.org/api/stories#hiding-stories-of-other-users) our stories).
...
[Mini apps](https://core.telegram.org/api/bots/webapps) may also request to share a story on the user's profile using the [web_app_share_to_story event »](https://core.telegram.org/api/web-events#web-app-share-to-story).
...
Stories may also be auto-pinned upon expiration if the `pinned` flag is set [when posting them](https://core.telegram.org/api/stories#posting-stories).

Additionally, stories pinned to a profile may **also** be pinned **to the top of the profile** using [stories.togglePinnedToTop](https://core.telegram.org/method/stories.togglePinnedToTop); stories that are pinned to the top of the profile (max [stories_pinned_to_top_count_max](https://core.telegram.org/api/config#stories-pinned-to-top-count-max)) should be displayed first when opening the profile.  
...
See [Live stories »](https://core.telegram.org/api/group-calls#live-stories) for the detailed API flow used to create, publish, join and interact with live stories.
...
Use [stories.createAlbum](https://core.telegram.org/method/stories.createAlbum) to create a story album.

Use [stories.getAlbums](https://core.telegram.org/method/stories.getAlbums) to get all story albums owned by a user.

Use [stories.getAlbumStories](https://core.telegram.org/method/stories.getAlbumStories) to fetch the stories added to an album.

Use the **albums** parameter of [stories.sendStory](https://core.telegram.org/method/stories.sendStory) to add a newly posted story to one or more albums.

Use [stories.updateAlbum](https://core.telegram.org/method/stories.updateAlbum) to rename an album, or add, delete and reorder stories in an album.

Use [stories.reorderAlbums](https://core.telegram.org/method/stories.reorderAlbums) to reorder albums on the profile.

Use [stories.deleteAlbum](https://core.telegram.org/method/stories.deleteAlbum) to delete an album.

A profile can contain a maximum of [stories_albums_limit »](https://core.telegram.org/api/config#stories-albums-limit) albums, each containing a maximum of [stories_album_stories_limit](https://core.telegram.org/api/config#stories-album-stories-limit) stories.

[Story album deep links »](https://core.telegram.org/api/links#story-album-links) can be used to share an album.
...
Active stories of contacts, subscribed channels/supergroups and the [changelog user](https://core.telegram.org/api/config#stories-changelog-user-id) should be shown in the action bar of the home screen.  
Use [stories.getAllStories](https://core.telegram.org/method/stories.getAllStories) to fetch the full list of active stories.

Optionally, the `hidden` flag can be set to fetch the [hidden stories to be shown in the archived tab](https://core.telegram.org/api/stories#hiding-stories-of-other-users), instead of the main story list.
...
The `stories_max_id` field of [user](https://core.telegram.org/constructor/user) and [channel](https://core.telegram.org/constructor/channel) contains a [recentStory](https://core.telegram.org/constructor/recentStory) summary of the peer's active stories.

If the summary has `live` set, `max_id` identifies the peer's active [live story »](https://core.telegram.org/api/stories#live-stories).

Otherwise, `max_id` is the maximum ID among the peer's active stories.

In any case, when `max_id` it is greater than the locally stored maximum read story ID for the peer, the peer has unread active stories. The maximum read story ID is stored in [peerStories](https://core.telegram.org/constructor/peerStories).`max_read_id` and updated by [updateReadStories](https://core.telegram.org/constructor/updateReadStories).

Use [stories.getPeerMaxIDs](https://core.telegram.org/method/stories.getPeerMaxIDs) to refresh these compact summaries for a list of peers without fetching full [StoryItem](https://core.telegram.org/type/StoryItem) constructors. In the result of [stories.getPeerMaxIDs](https://core.telegram.org/method/stories.getPeerMaxIDs), a [recentStory](https://core.telegram.org/constructor/recentStory) without any flag set means the peer has no active stories.
...
[stories.searchPosts](https://core.telegram.org/method/stories.searchPosts) may be used to globally search for stories from all users (even non-contacts and other peers we've never seen before) using either a hashtag passed to `hashtag` (without the `#`) or a [location tag](https://core.telegram.org/api/stories#location-tags) passed to `area`.

Global search should be automatically triggered when clicking on a hashtag in the description of a story, or when clicking on a [location tag](https://core.telegram.org/api/stories#location-tags) (results displayed under the map).
...
Upon encountering a story deep link, clients should open the specified story as [specified here »](https://core.telegram.org/api/stories#watching-stories).

See [here »](https://core.telegram.org/api/links#story-links) for more info on story deep links.

A [live story deep link »](https://core.telegram.org/api/links#live-story-links) can be used to open the currently ongoing live story of a user or channel, and a [post story deep link »](https://core.telegram.org/api/links#post-story-links) can be used to open the story composer.
...
Clients should fetch and display a copy of the channel post on top of the story according to the [media area](https://core.telegram.org/api/stories#media-areas) `coordinates`: clicking on the media area should open the linked post.
...
Location tags are represented by a [mediaAreaVenue](https://core.telegram.org/constructor/mediaAreaVenue) or [mediaAreaGeoPoint](https://core.telegram.org/constructor/mediaAreaGeoPoint), associated to a location sticker on top of the story media with an associated [clickable media area](https://core.telegram.org/api/stories#media-areas).
...
[mediaAreaGeoPoint](https://core.telegram.org/constructor/mediaAreaGeoPoint) is used to represent a simple geolocation with an *optional* textual address.  
...
[Global story search »](https://core.telegram.org/api/stories#searching-stories) should be automatically triggered when clicking on a location, passing the [mediaAreaGeoPoint](https://core.telegram.org/constructor/mediaAreaGeoPoint)/[mediaAreaVenue](https://core.telegram.org/constructor/mediaAreaVenue) to [stories.searchPosts](https://core.telegram.org/method/stories.searchPosts), with results displayed under the map.
...
If the [story_weather_preload](https://core.telegram.org/api/config#story-weather-preload) client configuration parameter is equal to true, clients should preload the weather using the flow specified above on startup (as opposed to only doing it when creating a weather media area).
...
Owned [collectible gifts »](https://core.telegram.org/api/gifts#collectible-gifts) may be shared in a story using a [mediaAreaStarGift](https://core.telegram.org/constructor/mediaAreaStarGift), containing the `slug` from [starGiftUnique](https://core.telegram.org/constructor/starGiftUnique).`slug`, that can be resolved as [specified here »](https://core.telegram.org/api/gifts#sharing-and-getting-info-about-a-collectible-gift).
