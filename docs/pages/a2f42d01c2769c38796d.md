...
Source: https://core.telegram.org/api/antispam
...
Once enabled, Telegram's native antispam system will start monitoring the supergroup, automatically deleting spam messages using the official Telegram Antispam bot, with ID equal to `telegram_antispam_user_id`, as specified by the [client configuration parameters](https://core.telegram.org/api/config#client-configuration).  
Note that this bot **does not** have to be added as participant or admin to the supergroup for the native antispam system to work, just invoking [channels.toggleAntiSpam](https://core.telegram.org/method/channels.toggleAntiSpam) is enough; however, it should still be locally shown as a group admin when displaying the admin list to the user (clients should simply append it to the admin list *without* invoking [channels.editAdmin](https://core.telegram.org/method/channels.editAdmin)).
