...
If true, clients should preload the current weather on startup (as opposed to only when creating a [weather media area](https://core.telegram.org/api/stories#weather)) by querying the bot specified in [weather_search_username](https://core.telegram.org/api/config#weather-search-username). (boolean)
...
Specifies the name of the service providing GIF search through [gif_search_username](https://core.telegram.org/api/config#mtproto-configuration) (string)
...
Maximum number of unique reactions for any given message: for example, if there are 2000 ![👍](//telegram.org/img/emoji/40/F09F918D.png) and 1000 custom emoji ![😁](//telegram.org/img/emoji/40/F09F9881.png) reactions and reactions_uniq_max = 2, you can't add a ![👎](//telegram.org/img/emoji/40/F09F918E.png) reaction, because that would raise the number of unique reactions to 3 > 2. (integer)
...
Specifies a list of allowed schemes for URLs received in [web_app_open_link events](https://core.telegram.org/api/web-events#web-app-open-link). (array of strings)
...
When choosing the price for [paid direct channel messages](https://core.telegram.org/api/paid-messages), the UI should display [stars_paid_messages_channel_amount_default](https://core.telegram.org/api/config#stars-paid-messages-channel-amount-default) as the initial default value.
