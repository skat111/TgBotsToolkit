...
Source: https://core.telegram.org/api/end-to-end%2Fpfs
...
A (re-keying initiator) generates a new value of *a*, subject to the same limitations as for the [initial Diffie-Hellman key exchange](https://core.telegram.org/api/end-to-end#key-generation), and sends the value of *pow(g,a) mod p* to B, embedded in a [decryptedMessageService](https://core.telegram.org/constructor/decryptedMessageService):
...
There is one exception to this rule — the SHA-1 of the original key (generated during the establishment of Secret Chat in question) is always stored, in order to show [key visualizations](https://core.telegram.org/api/end-to-end%2Fpfs#key-visualization) on the clients.
