...
2. Then, the prepared inline message ID is passed to the user's client via the [web_app_send_prepared_message web event »](https://core.telegram.org/api/web-events#web-app-send-prepared-message).
3. The ID is then passed to [messages.getPreparedInlineMessage](https://core.telegram.org/method/messages.getPreparedInlineMessage) along with the identifier of the bot that owns the web app to fetch the prepared inline message  
   3.1) If the method returns any RPC error, emit a [prepared_message_failed event](https://core.telegram.org/api/bots/webapps#prepared-message-failed) with `error` set to the message of the RPC error and abort the process.  
   3.2) If the method succeeds, display a preview to the user, letting them select a destination chat.
4. If the user aborts the process, emit a [prepared_message_failed event](https://core.telegram.org/api/bots/webapps#prepared-message-failed) with `error` set to `USER_DECLINED`.
5. If the user selects a chat, send the message [as specified here »](https://core.telegram.org/api/bots%2Finline#3-sending-the-inline-query-result).  
   5.1) If sending fails, emit a [prepared_message_failed event](https://core.telegram.org/api/bots/webapps#prepared-message-failed) with `error` set to `MESSAGE_SEND_FAILED`.  
   5.2) If sending succeeds, emit a [prepared_message_sent event](https://core.telegram.org/api/bots/webapps#prepared-message-sent).
