...
Source: https://core.telegram.org/api/stickers
...
The [documentAttributeSticker](https://core.telegram.org/constructor/documentAttributeSticker) attribute contains information about the associated [stickerset](https://core.telegram.org/api/stickers#stickersets), the emoji that represents the sticker, and more.

There are multiple available sticker types:

* [Static stickers »](https://core.telegram.org/api/stickers#static-stickers)
* [Animated stickers »](https://core.telegram.org/api/stickers#animated-stickers)
* [Video stickers »](https://core.telegram.org/api/stickers#video-stickers)

Like all files, stickers have a set of previews that should be handled as described [here »](https://core.telegram.org/api/files#handling-audio-video-and-vector-previews).

Stickers are organized in [stickersets »](https://core.telegram.org/api/stickers#stickersets), and are also used in the following contexts:
...
* [Mask stickers »](https://core.telegram.org/api/stickers#mask-stickers)

A single stickerset can contain a mix of [static](https://core.telegram.org/api/stickers#static-stickers), [animated](https://core.telegram.org/api/stickers#animated-stickers) and [video](https://core.telegram.org/api/stickers#video-stickers) stickers.
...
See [here »](https://core.telegram.org/stickers#static-stickers-and-emoji) for tips on how to create the perfect static sticker, and [here »](https://core.telegram.org/api/stickers#creating-stickersets) for info on how to upload it using the API.
...
See [here »](https://core.telegram.org/stickers#creating-animations) for tips on how to create the perfect animated sticker, and [here »](https://core.telegram.org/api/stickers#creating-stickersets) for info on how to upload it using the API.
...
Premium stickers may only be *sent* by users with a [Premium](https://core.telegram.org/api/premium) subscription, and the animated effect should be played by all users, including those without a Premium subscription.
...
See [here »](https://core.telegram.org/stickers#video-stickers-and-emoji) for tips on how to create the perfect video sticker, and [here »](https://core.telegram.org/api/stickers#creating-stickersets) for info on how to upload it using the API.
...
Also, like all files, sticker [documents](https://core.telegram.org/constructor/document) have a set of previews that should be handled as described [here »](https://core.telegram.org/api/files#handling-audio-video-and-vector-previews).
...
A [stickerset](https://core.telegram.org/api/stickers#stickersets) can be installed using [messages.installStickerSet](https://core.telegram.org/method/messages.installStickerSet), `archived=false`, with possible return values:
...
* [messages.stickerSetInstallResultArchive](https://core.telegram.org/constructor/messages.stickerSetInstallResultArchive) - The stickerset was successfully installed, displacing some older unused stickersets specified in the `sets` field by [archiving them](https://core.telegram.org/api/stickers#archived-stickersets).
...
Users and bots can directly modify stickersets created using [stickers.createStickerSet](https://core.telegram.org/method/stickers.createStickerSet) and [@stickers](https://t.me/stickers): stickersets owned by the current user/bot will have the `creator` flag set.
...
Use [stickers.createStickerSet](https://core.telegram.org/method/stickers.createStickerSet) to create normal, [mask](https://core.telegram.org/api/stickers#mask-stickers) and [custom emoji](https://core.telegram.org/api/custom-emoji) stickersets, *also* when [importing stickers using the stickers SDK](https://core.telegram.org/import-stickers).
...
Favorited stickers can be fetched using [messages.getFavedStickers](https://core.telegram.org/method/messages.getFavedStickers): note that this method returns a set of [stickerset previews »](https://core.telegram.org/api/stickers#stickerset-previews).
...
An unused stickerset may also be automatically archived when [installing new stickersets](https://core.telegram.org/api/stickers#installing-stickersets).

Archived stickers are not returned by [messages.getAllStickers](https://core.telegram.org/method/messages.getAllStickers) and can only be fetched using [messages.getArchivedStickers](https://core.telegram.org/method/messages.getArchivedStickers): note that this method returns a set of [stickerset previews »](https://core.telegram.org/api/stickers#stickerset-previews).
...
After using a sticker in a message, clients should invoke [messages.saveRecentSticker](https://core.telegram.org/method/messages.saveRecentSticker) (normal and [mask](https://core.telegram.org/api/stickers#mask-stickers) stickers only, mask stickers should set the `attached` flag).
...
The `update_stickersets_order` flag should be set when manually choosing and using stickers, [masks](https://core.telegram.org/api/stickers#mask-stickers) and [custom emojis](https://core.telegram.org/api/custom-emoji) from a specific stickerset in the stickerset selection bar (not through suggested or recent stickersets UI views).  
...
Note that the API also offers a separate list of recent *stickers* (not stickersets), see [here »](https://core.telegram.org/api/stickers#recent-stickers) for more info.
...
To fetch this special list, invoke [messages.getStickers](https://core.telegram.org/method/messages.getStickers) with `emoticon=![⭐️](//telegram.org/img/emoji/40/E2AD90.png)![⭐️](//telegram.org/img/emoji/40/E2AD90.png)`
...
First, overlay the sticker on top of the media file locally (properly handling eventual default [mask coordinates](https://core.telegram.org/api/stickers#mask-stickers)), and [upload](https://core.telegram.org/api/files) the modified media file.  
...
When receiving such a media file, clients should call [messages.getAttachedStickers](https://core.telegram.org/method/messages.getAttachedStickers), passing the media: the method will return a set of [stickerset previews »](https://core.telegram.org/api/stickers#stickerset-previews), with info about the stickersets (not stickers) used in the media.

#### Mask stickers

Mask stickers are a special kind of [static](https://core.telegram.org/api/stickers#static-stickers) stickers that are meant to be overlaid on top of photos and attached to media, as specified [here »](https://core.telegram.org/api/stickers#attached-stickers).
...
Note that these coordinates are only used to provide a default position when [attaching stickers to media](https://core.telegram.org/api/stickers#attached-stickers), by locally running facial recognition software and placing the mask sticker at the appropriate coordinates relative to the chosen facial feature.  
...
The default coordinates are chosen by the stickerset creator when [uploading the sticker](https://core.telegram.org/api/stickers#creating-stickersets).