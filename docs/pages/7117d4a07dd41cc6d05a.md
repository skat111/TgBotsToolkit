...
Source: https://core.telegram.org/api/bots/ai
...
* Chat ID
* Topic ID (for [bot forums](https://core.telegram.org/api/bots/ai#bot-forums))
...
To recap, each chat (or each bot forum topic, for [bot forums »](https://core.telegram.org/api/bots/ai#bot-forums)) can have a variable number of live drafts, with each new draft replacing the previous one only if it has the same `random_id` and is sent to the same topic and peer.
