...
Source: https://core.telegram.org/api/bots%2Fids

The `id` field of [peers »](https://core.telegram.org/api/peers) (i.e. users, chats, channels, secret chats) contains four distinct sequences of unique 64-bit IDs used to identify a specific user, chat, channel or secret chat.
...
**Note**: the above range does **not** apply to [monoforums »](https://core.telegram.org/api/monoforum), which are a special kind of forum used for direct channel messages, represented by [channel](https://core.telegram.org/constructor/channel) constructors with the `monoforum` flag set: see [here »](https://core.telegram.org/api/bots%2Fids#monoforum-ids) for the range used by monoforums.

### Monoforum IDs

[Monoforums »](https://core.telegram.org/api/monoforum) are a special kind of forum used for direct channel messages, represented by [channel](https://core.telegram.org/constructor/channel) constructors with the `monoforum` flag set.

The MTProto IDs of monoforums range from `1002147483649` to `3000000000000`.

To convert MTProto monoforum IDs to bot API monoforum IDs, add `1000000000000` and make them negative (and vice versa): this procedure is the same used for [supergroup/channel IDs](https://core.telegram.org/api/bots%2Fids#supergroup-channel-ids), the only change is the source MTProto range.
