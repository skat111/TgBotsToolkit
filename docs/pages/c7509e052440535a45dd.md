...
Source: https://core.telegram.org/api/stars
...
Clients should disable Telegram Stars support for the current user if the [stars_purchase_blocked field is equal to `true` »](https://core.telegram.org/api/config#stars-purchase-blocked) due to regional limitations.
...
The channel/bot ad revenue transaction history and balance in **nanograms** (`1/1_000_000_000`th of a Gram) may also be fetched by channel/bot admins by setting the `ton` flag of all methods listed above, if the [channelFull](https://core.telegram.org/constructor/channelFull).`can_view_revenue`/[userFull](https://core.telegram.org/constructor/userFull).`can_view_revenue` flag is set.
...
Changes in the star balance (through [topups](https://core.telegram.org/api/stars#buying-or-gifting-stars) or [purchases](https://core.telegram.org/api/stars#using-stars)) will be notified by the server via an [updateStarsBalance](https://core.telegram.org/constructor/updateStarsBalance) update.
...
Channel/bot ad revenue statistics in **nanograms** (`1/1_000_000_000`th of a Gram) may be also fetched by channel/bot admins by setting the `ton` flag, if the [channelFull](https://core.telegram.org/constructor/channelFull).`can_view_revenue`/[userFull](https://core.telegram.org/constructor/userFull).`can_view_revenue` flag is set.

Use [payments.getStarsRevenueStats](https://core.telegram.org/method/payments.getStarsRevenueStats) to fetch statistics about earned Telegram Stars/nanograms; the returned [StatsGraph](https://core.telegram.org/type/StatsGraph) graphs can be rendered [as described here »](https://core.telegram.org/api/stats#graph-visualization).
...
**Note**: Star gifting functionality should only be enabled if the [stars_gifts_enabled »](https://core.telegram.org/api/config#stars-gifts-enabled) flag is equal to true.
...
The `spend_purpose_peer` of [inputStorePaymentStarsTopup »](https://core.telegram.org/constructor/inputStorePaymentStarsTopup) should be populated with the peer where the topup process was initiated due to low funds (i.e. a bot for bot payments, a channel for paid media/reactions, etc); leave this flag unpopulated if the topup flow was not initiated when attempting to spend more Stars than currently available on the account's balance.
...
Purchasing [gifts](https://core.telegram.org/api/stars#star-gifts), sending [Paid Messages](https://core.telegram.org/api/stars#paid-messages) and funding [Suggested Posts](https://core.telegram.org/api/stars#suggested-posts) with Stars increases your rating. However, if you refund Star purchases or [convert gifts to Stars](https://core.telegram.org/api/gifts) your rating will decrease.

The rating is contained in the [starsRating](https://core.telegram.org/constructor/starsRating) constructor, from [userFull](https://core.telegram.org/constructor/userFull).`stars_rating`.
...
We may also see the pending star rating in [userFull](https://core.telegram.org/constructor/userFull).`stars_my_pending_rating` only for ourselves, which will be applied at [userFull](https://core.telegram.org/constructor/userFull).`stars_my_pending_rating_date`.

[stars_rating_learnmore_url](https://core.telegram.org/api/config#stars-rating-learnmore-url) will contain a URL that can be shown to users, leading to a page with more info about the Star Rating system.
...
Channel owners can then [withdraw Stars as Gram »](https://core.telegram.org/api/stars#withdrawing-revenue), or [use Stars to place ads for the channel »](https://core.telegram.org/api/stars#paying-for-ads).

[Star revenue statistics](https://core.telegram.org/api/stars#revenue-statistics) and [balance information](https://core.telegram.org/api/stars#balance-and-transaction-history) is also available to channel owners.

#### Paid messages

Telegram Stars can be used to pay for sending messages to users and channels that have configured [paid messages »](https://core.telegram.org/api/paid-messages), requiring a payment for every message sent to them.
...
See [here »](https://core.telegram.org/api/suggested-posts) for the full flow.
...
Bot owners can then [withdraw Stars as Gram »](https://core.telegram.org/api/stars#withdrawing-revenue), or [use Stars to place ads for the channel »](https://core.telegram.org/api/stars#paying-for-ads).

[Star revenue statistics](https://core.telegram.org/api/stars#revenue-statistics) and [balance information](https://core.telegram.org/api/stars#balance-and-transaction-history) is also available to bot owners.
...
Only the channel/bot owner can invoke this method, and only if the balance is bigger than or equal to [stars_revenue_withdrawal_min »](https://core.telegram.org/api/config#stars-revenue-withdrawal-min) and the `status.withdrawal_enabled` flag returned by [payments.getStarsRevenueStats](https://core.telegram.org/method/payments.getStarsRevenueStats) is set.

[stars_revenue_withdrawal_max »](https://core.telegram.org/api/config#stars-revenue-withdrawal-max) specifies the maximum amount of [Telegram Stars](https://core.telegram.org/api/stars) that can be [withdrawn »](https://core.telegram.org/api/stars#withdrawing-revenue) from a channel or bot's balance.  
...
[Transactions](https://core.telegram.org/api/stars#balance-and-transaction-history) for ad payments will be of type [starsTransactionPeerAds](https://core.telegram.org/constructor/starsTransactionPeerAds).
...
[inputInvoiceBusinessBotTransferStars](https://core.telegram.org/constructor/inputInvoiceBusinessBotTransferStars) may be used to transfer stars from the balance of a user account [connected to a business bot](https://core.telegram.org/api/bots/connected-business-bots), to the balance of the business bot.

The number of available stars on the user's balance may be fetched by invoking [payments.getStarsStatus](https://core.telegram.org/method/payments.getStarsStatus) with peer=[inputPeerSelf](https://core.telegram.org/constructor/inputPeerSelf) over the business connection.

To transfer the stars, invoke [payments.getPaymentForm](https://core.telegram.org/method/payments.getPaymentForm) with [inputInvoiceBusinessBotTransferStars](https://core.telegram.org/constructor/inputInvoiceBusinessBotTransferStars) (with `bot`=[inputPeerSelf](https://core.telegram.org/constructor/inputPeerSelf)), and follow the usual [payment flow »](https://core.telegram.org/api/payments#22-getting-invoice-info-about-the-product), **with all method calls executed over the business connection**.

### My Stars deep links

[My Stars deep links »](https://core.telegram.org/api/links#my-stars-links) and [my Gram deep links »](https://core.telegram.org/api/links#my-grams-links) can be used to bring the user to the My Stars/My Gram page, containing the topup button, the Star/TON transaction history, statistics, and much more, as specified in the above sections.
...
In some places in the API, it's possible to use Grams instead of Stars: in this case, a [starsTonAmount](https://core.telegram.org/constructor/starsTonAmount) constructor will be used instead of [starsAmount](https://core.telegram.org/constructor/starsAmount), specifying an amount of TONs in the smallest unit of the cryptocurrency, currently *nanograms*, `1/1_000_000_000`th (a billionth) of a Gram.

When TONs are gifted to the user, a [messageActionGiftTon](https://core.telegram.org/constructor/messageActionGiftTon) is received: this constructor, along with its contents, should be displayed next to the appropriate sticker from the [inputStickerSetTonGifts »](https://core.telegram.org/constructor/inputStickerSetTonGifts) [stickerset »](https://core.telegram.org/api/stickers#stickersets):
